...
public class CameraScrıpt : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        if (camState == CameraState.OnGame)
        {
            ...
            //Debug.Log(targetRot);
            targetRot.y = /*(transform.rotation * Quaternion.AngleAxis(*/
                ( /* transform.rotation.y - */followedObject.position.x) * Time.deltaTime; //, Vector3.up)).y;
            //Debug.Log(targetRot);
    ...
    IEnumerator RotateOnStart()
    {
        ...
        {
            if (Vector3.Distance(transform.position, GamePlayPosition) > 2f
            ) //!Mathf.Approximately(transform.position.sqrMagnitude, GamePlayPosition.sqrMagnitude))
            {
            ...
            if (Vector3.Distance(transform.rotation.eulerAngles, GamePlayRotation.eulerAngles) > 5f
            ) //!Mathf.Approximately(transform.rotation.eulerAngles.sqrMagnitude, GamePlayRotation.eulerAngles.sqrMagnitude))
            {
    ...
    IEnumerator SpanToPosition(Transform spannedObj, float speed, float downModifier = 0f)
    {
        ...
        while (!spanned)
        {
            ...
            float distanceToSpan =
                Vector3.Distance(transform.position, spanPos + (Vector3.down * downModifier) + offset);

            if (distanceToSpan > 1f)
            {
                transform.position = Vector3.Lerp(transform.position, spanPos + (Vector3.down * downModifier) + offset,
                    speed / 80f);
    ...
    IEnumerator ReturnOldRotationSmooth()
    {
        ...
        bool returned = false;
        float targetAngle = followedObject.transform.forward.z < 0.0f ? 180f : 0f;
        float totalAngles = FixEuler(targetAngle - FixEuler(transform.eulerAngles.y));
        ...
        {
            //float rot = Mathf.LerpAngle(transform.eulerAngles.y, (targetAngle), Time.deltaTime);

            //Quaternion targetRot = Quaternion.AngleAxis(rot, Vector3.up);

            Quaternion targetRot = Quaternion.AngleAxis((speed * Time.deltaTime * totalAngles), Vector3.up);
            ...
            {
                targetRot = Quaternion.AngleAxis(targetAngle - FixEuler(transform.eulerAngles.y), Vector3.up);

                Debug.Log($"ReturnOldRotationSmooth looked angle : {transform.eulerAngles.y} and totalAngle {totalAngles} and totRitated {totRoatedAmount} and oldrotation is {targetAngle}");
        ...
        rotated = !(targetAngle < 90f);
        doNotRotateCamera = false;
        camState = CameraState.Follow;
        StopCoroutine(camMovement);
        camMovement = null;
    }
    ...
    IEnumerator LookAtSmoothCorountine()
    {
        ...
        ResetFollowParameters();
        
        //oldRotation = FixEuler(transform.eulerAngles.y);
        bool looked = false;

        float fixedCurrAngle = FixEuler(transform.eulerAngles.y);
        float desiredAngle = FixEuler(followedObject.eulerAngles.y); // followedObject.transform.eulerAngles.y; 
        float totalAngles = FixEuler((desiredAngle) - fixedCurrAngle);
        
        Debug.Log($"LookAtSmoothCorountine initial angle : {transform.eulerAngles.y} and initial total angle {totalAngles}");
        Debug.Log($"LookAtSmoothCorountine initial desired : {followedObject.eulerAngles.y} and current desired angle {desiredAngle}");
        
        if (desiredAngle < 0f && Mathf.Abs(desiredAngle) > (transform.eulerAngles.y)  &&  Mathf.Abs(desiredAngle) < fixedCurrAngle && !Mathf.Approximately(Mathf.Abs(fixedCurrAngle), 0f) && !Mathf.Approximately(Mathf.Abs(fixedCurrAngle),180f))
            totalAngles = 180f - Mathf.Abs(totalAngles);
        else if ((desiredAngle > transform.eulerAngles.y && transform.eulerAngles.y > 0f) && transform.eulerAngles.y > 180f && !Mathf.Approximately(transform.eulerAngles.y,180f))
            totalAngles *= -1f;
    ...
    private bool doNotRotateCamera = false;
    ...
    Quaternion LookAtTarget()
    {
        ...
        var rotatedAgainWhileRotating =
            rotating && ((rotated == false && direction > 0.2f) || (rotated && direction < -0.2f));
        ...
        if (rotatedBackward || rotateForward)
        {
            ...
            if (Mathf.Abs(totalRotation) >= Mathf.Abs(rotateForwardAmount))
            {
                ...
                targetRot = Quaternion.AngleAxis(rotateForwardAmount - totalRotation, Vector3.up);
                ResetFollowParameters();
                return targetRot;
            }
        ...
        else if (rotatedAgainWhileRotating)
        {
            ...
            if (totalRotation <= 0f)
            {
                Debug.LogError($" donme bitti {transform.eulerAngles.y } and total rot is {totalRotation} and rotationDegreesAmount { rotationDegreesAmount}");
                ResetFollowParameters();
                targetRot = Quaternion.AngleAxis(totalRotation, Vector3.up);
                return targetRot;
            }
